...
export default function Commissions() {
  ...
  const [dueUser, setDueUser] = useState('')
  const [dueGroup, setDueGroup] = useState('')
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [cRes, uRes, gRes, grpRes, gcRes] = await Promise.all([
      ...
      if (!cRes.ok || !uRes.ok || !gRes.ok || !grpRes.ok || !gcRes.ok) throw new Error()
  ...
  async function setDue() {
    if (!dueUser.trim()) return toast('请填写用户名', 'error')
    if (!dueGroup) return toast('请选择游戏组', 'error')
    const res = await apiFetch('/api/admin/group-commissions', {
      method: 'POST',
      body: JSON.stringify({
        user_name: dueUser.trim(),
        game_group_id: Number(dueGroup),
        total_count: parseInt(dueCount) || 0,
      }),
    })
    if (res.ok) {
      setDueUser('')
  ...
  return (
    <Box>
      <Card sx={{ mb: 2 }}>
        <CardHeader title="新增记录" titleTypographyProps={{ variant: 'h6' }} />
        <CardContent>
            ...
            <Box sx={{ flex: 1 }}>
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                <TextField
                  label="用户名/别名"
                  value={dueUser}
                  onChange={(e) => setDueUser(e.target.value)}
                  sx={{ flex: 1 }}
                />
                <TextField
                  select
                  label="游戏组"
                  value={dueGroup}
                  onChange={(e) => setDueGroup(e.target.value)}
                  sx={{ flex: 1 }}
                >
                  {groups.map((g) => (
                ...
                <TextField
                  ...
                  type="number"
                  value={dueCount}
                  onChange={(e) => setDueCount(e.target.value)}
                  sx={{ flex: 1 }}
                />
                <Button variant="contained" onClick={setDue} sx={{ alignSelf: 'center' }}>
                  设置
                </Button>
              </Stack>
            </Box>
        ...
        <Card key={ug.user_id} sx={{ mb: 2 }}>
          <CardHeader
            ...
            subheader={`应得合计 ${ug.groups.reduce((s, g) => s + g.total, 0)} · 已完合计 ${ug.groups.reduce(
              (s, g) => s + g.completed,
              0,
            )} · 已打卡 ${ug.groups.reduce((s, g) => s + g.checkedCount, 0)}/${records.filter((r) => r.user_id === ug.user_id).length}`}
          />
          <Divider />
